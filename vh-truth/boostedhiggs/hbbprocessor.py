...
#                hist.Bin('n2ddt2', r'Jet 2 N2ddt value', [-0.25, 0, 0.25]),
#                hist.Bin('pt2', r'Jet 2 $p_{T}$ [GeV]', [200, 450, 1200]),
#            ),
            'templates2': hist.Hist(
                'Events',
                hist.Cat('dataset', 'Dataset'),
                hist.Cat('region', 'Region'),
                hist.Bin('pt1', r'Truthjet 1 $p_{T}$ [GeV]', 20,200,1200),
                hist.Bin('pt2', r'Truthjet 2 $p_{T}$ [GeV]', 20,200,1200),
                hist.Bin('deltaR', r'Truthjet $\Delta R$', 24,0,6),
            ),
            'templates3': hist.Hist(
                'Events',
                hist.Cat('dataset', 'Dataset'),
                hist.Cat('region', 'Region'),
                hist.Bin('pt1', r'Particle 1 $p_{T}$ [GeV]', 20,200,1200),
                hist.Bin('pt2', r'Particle 2 $p_{T}$ [GeV]', 20,200,1200),
                hist.Bin('particle1', r'Particle 1 PDG ID', 3,23,26),
                hist.Bin('particle2', r'Particle 2 PDG ID', 3,23,26),
                hist.Bin('deltaR', r'Particle $\Delta R$', 24,0,6),
            ),
...
#                hist.Cat('systematic', 'Systematic'),
    ...
    def process(self, events):
        ...
        truthpart = events.GenPart
        particles = truthpart[
            (truthpart.statusFlags == 10497)
            & (abs(truthpart.pdgId) > 22)
            & (abs(truthpart.pdgId) < 26)
        ]

        if self._jet_arbitration == 'pt':
            idx = (particles.pt).argsort()
            idx0 = idx[:,0:1]
            idx1 = idx[:,1:2]
            particle2 = particles[idx1]
            particle1 = particles[idx0]

        print(dataset, particles.counts)

        particle_pair = particle1.cross(particle2, nested=False)
        particle_dphi = abs(particle_pair.i0.delta_phi(particle_pair.i1))
        particle_deta = abs(particle_pair.i0.eta - particle_pair.i1.eta)
        particle_dR = np.sqrt(particle_dphi*particle_dphi + particle_deta*particle_deta)

        truthjets = events.GenJetAK8
        if self._jet_arbitration == 'pt':
            idx = (truthjets.pt).argsort()
            idx0 = idx[:,0:1]
            idx1 = idx[:,1:2]
            truthjet2 = truthjets[idx1]
            truthjet1 = truthjets[idx0]

        truthjet_pair = truthjet1.cross(truthjet2, nested=False)
        truthjet_dphi = abs(truthjet_pair.i0.delta_phi(truthjet_pair.i1))
        truthjet_deta = abs(truthjet_pair.i0.eta - truthjet_pair.i1.eta)
        truthjet_dR = np.sqrt(truthjet_dphi*truthjet_dphi + truthjet_deta*truthjet_deta)
...
#            )
            output['templates2'].fill(
                dataset=dataset,
                region=region,
                pt1=normalize(truthjet1.pt, cut),
                pt2=normalize(truthjet2.pt, cut),
                deltaR=normalize(truthjet_dR, cut),
                weight=weight,
            ),
            output['templates3'].fill(
                dataset=dataset,
                region=region,
                pt1=normalize(particle1.pt, cut),
                particle1=normalize(abs(particle1.pdgId), cut),
                pt2=normalize(particle2.pt, cut),
                particle2=normalize(abs(particle2.pdgId), cut),
                deltaR=normalize(particle_dR, cut),
                weight=weight,
            ),
